  ...
  (function() {
    ...
    (function() {
        $(document).ready(function() {
            ...
            var options = $.extend(true, {}, defaults, {
                initComplete: function() {
                    this.api().columns().every(function() {
                        ...
                        if( (column.index() == 0)||(column.index() == 3)||(column.index() == 4)||(column.index() == 5)||(column.index() == 6)||(column.index() == 7)||(column.index() == 8) )return;

                        if(column.index() == 1)
                            var placeholder='Ticket Filter';

                        if(column.index() == 2)
                            var placeholder='Consignor Filter';



                        if(column.index() == 9)
                            var placeholder='Status Filter';



                        var select = $('<select class="form-control width-full"><option value="">'+placeholder+'</option></select>')
                            .appendTo($(column.footer()).empty())
                            .on('change', function() {
                                var val = $.fn.dataTable.util.escapeRegex(
                                    $(this).val()
            ...
            $('#exampleTableSearch1').DataTable(options);
        });
    })();
    ...
    (function() {
        $(document).ready(function() {
            ...
            var options = $.extend(true, {}, defaults, {
                initComplete: function() {
                    this.api().columns().every(function() {
                        var column = this;
                        var some = column.index();
                        if( (column.index() == 0)||(column.index() == 5)||(column.index() == 6)||(column.index() == 7)||(column.index() == 8)||(column.index() == 9) )return;

                        if(column.index() == 1)
                            var placeholder='Ticket Filter';

                        if(column.index() == 2)
                            var placeholder='Ticket Date Filter';
                        if(column.index() == 3)
                            var placeholder='Destination Filter';

                        if(column.index() == 4)
                            var placeholder='Agent Filter';
            ...
            $('#exampleTableSearch2').DataTable(options);
        });
    })();
        ...
        $row.children('td').each(function(i) {
          ...
          if ($this.hasClass('actions')) {
            _self.rowSetActionsEditing($row);
              ...
              $('.gradeA td:first-child input').attr('readonly', 'readonly'); //readonly flight...
...
var list = [];
        ...
        values = $row.find('td').map(function() {
		  ...
		  list.push($.trim($this.find('input').val()));
		...
		var fname = $("#fn"+$(this).attr("data")+" input").val();
		$.ajax({
		type: "POST",
		url: '/flight/3',
		data:{code:list[0],name:list[1]},
		success: function(response) {
		 location.reload();
		}
		});
